...
export const assignPaper = withAdminAuth(async (user, paperId: string, chairId: string) => {
  try {
    ...
    if (!chair) {
      return { success: false, error: "Target chair not found or is not an active session chair." };
    }
    ...
    if (existingActiveAssignment && existingActiveAssignment.chairId !== parsed.data.chairId) {
      return { success: false, error: "This paper is already assigned to a session chair. Only one chair per paper is allowed." };
    }
    ...
    if (existing) {
      if (existing.status === "REMOVED") {
        ...
        return { success: false, error: "Paper is already assigned to this chair." };
      }
    } else {
    ...
    console.error("assignPaper error:", err);
    return { success: false, error: err.message || "Failed to assign paper" };
  }
});
